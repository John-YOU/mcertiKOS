 ...
 * For example, ping, pong, ding and fork correspond to the elf_ids
...
void sys_spawn(void)
{
    ...
    if(elf_id<1 || elf_id>4){
        ...
        //dprintf("---%d\n", elf_addr);
	      if (elf_id==1)
	          elf_addr=(unsigned int)_binary___obj_user_pingpong_ping_start;
	      else if (elf_id==2)
	          elf_addr=(unsigned int)_binary___obj_user_pingpong_pong_start;
	      else if (elf_id==3)
	          elf_addr=(unsigned int)_binary___obj_user_pingpong_ding_start;
        else if (elf_id==4)
            elf_addr=(unsigned int)_binary___obj_user_fork_fork_start;
        }
        //dprintf("%d\n", elf_addr);
        unsigned int pid = proc_create((void*)elf_addr, quota);
        syscall_set_errno(E_SUCC);
...
{
    // TODO
    unsigned int curid, childid, quota, vaddr;
    curid = get_curid();
    quota = container_get_quota(curid); 
    childid = proc_fork(quota);
    //dprintf("parentid - childid - quota:%d %d %d\n",curid,childid,quota);
    
    copy_pdir_ptbl(curid, childid);
    //dprintf("Now go back to sys_fork \n");
    syscall_set_errno(E_SUCC);
    //dprintf("set errno succeed]\n");
    syscall_set_retval1(childid);
    //dprintf("Bu hui shi zhe li ba \n");
}
