...
// Implement the palloc function using LinkedList
unsigned int mypalloc(){
    unsigned int idx = rm_from_free();
    if(idx==0)return 0;
    add_to_alloc(idx);
    return idx;
}

// Implement the pfree function using LinkedList
void mypfree(unsigned int pfree_index){
    rm_from_alloc(pfree_index);
    add_to_free(pfree_index);
}
