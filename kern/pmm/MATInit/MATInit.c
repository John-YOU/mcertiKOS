...
/*
 * Initialize the lists for free and allocated pages.
 * Traverse the memory map, add unallocated pages to the free page list.
 * Add allocated pages to allocated page list.
 * Skip the pages that are reserved for kernel use.
 */
void set_list(unsigned int nps){
    init_list();
    unsigned int pg_idx = VM_USERLO_PI;
    while (pg_idx < VM_USERHI_PI) {
        if(at_is_allocated(pg_idx)){
            add_to_alloc(pg_idx);
        }
        else{
            add_to_free(pg_idx);
